    ...
    <div className="flex h-screen items-center justify-center bg-white dark:bg-gh-canvas">
      <div className="w-[420px] max-w-[calc(100vw-32px)] border border-gh-border rounded-xl shadow-md p-7 bg-white dark:bg-gh-canvas">
        <div className="text-center mb-6">
          ...
          <p className="text-ui-caption text-gh-fg-muted mt-1">管理你的 GitHub Star 仓库</p>
        </div>
        ...
        {candidates.length > 1 && validUser
          ? (
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-gh-canvas rounded-lg border border-gh-border-muted">
                  ...
                  <div>
                    <p className="text-ui-body font-medium text-gh-fg">{validUser.name || validUser.login}</p>
                    <p className="text-ui-caption text-gh-fg-muted">检测到多个 GitStars Gist，请选择一个</p>
                  </div>
                </div>
                {candidates.map(candidate => (
                  ...
                  >
                    <code className="block text-ui-caption text-gh-accent mb-1">{candidate.id}</code>
                    <span className="text-ui-caption text-gh-fg-muted">{getCandidateSummary(candidate)}</span>
                  </button>
                ))}
                <button type="button" className="w-full text-ui-caption text-gh-fg-muted" onClick={() => setCandidates([])}>
                  使用其他 PAT
                </button>
              </div>
            )
          : (
              <div className="space-y-4">
                <div>
                  <label className="block text-ui-body font-medium text-gh-fg mb-1.5">Personal Access Token</label>
                  <input
                    type="password"
                    className="gh-input text-ui-body"
                ...
                <div className="bg-gh-canvas rounded-lg border border-gh-border-muted p-3 text-ui-caption text-gh-fg-muted space-y-1.5">
                  ...
                  {[
                    ...
                    <div key={scope} className="flex items-center gap-2">
                      ...
                      <span>
                        <code className="bg-gh-canvas-subtle px-1 rounded text-ui-caption">{scope}</code>
                        {' '}
                        {label}
                      </span>
                    </div>
                ...
                {error && <div className="text-ui-caption text-gh-danger bg-gh-danger-subtle rounded-lg border border-gh-danger/20 p-3">{error}</div>}
                <button
                  type="button"
